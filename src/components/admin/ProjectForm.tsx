...
export default function ProjectForm({ initialData }: ProjectFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    media: initialData?.media?.[0]?.id ? [initialData.media[0].id] : [] as string[],
  ...
  const handleAddTag = () => {
    if (tagInput && !formData.tech_tags.includes(tagInput)) {
      setFormData({ ...formData, tech_tags: [...formData.tech_tags, tagInput] });
      setTagInput("");
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 max-w-2xl mx-auto">
      ...
      <div>
        <label className="text-xs font-bold text-zinc-500 uppercase mb-2 block">Hero Image</label>
      ...
      <div>
        <label className="text-xs font-bold text-zinc-500 uppercase mb-2 block">Tech Stack</label>
        ...
        <div className="flex flex-wrap gap-2">
          {formData.tech_tags.map(tag => (
            <span key={tag} className="bg-rose-500/10 text-rose-500 px-3 py-1 rounded-full text-sm font-bold flex items-center gap-2">
              {tag}
      ...
      <div className="grid gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
           <div>
              <label className="text-xs font-bold text-zinc-500 uppercase">Live URL</label>
              <input 
